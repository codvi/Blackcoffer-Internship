...
const IntensityChart = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const colorScale = d3
      .scaleThreshold()
      .domain([
        d3.max(intensityData) / 4,
        (d3.max(intensityData) / 4) * 2,
        (d3.max(intensityData) / 4) * 3,
      ])
      .range(["#2563eb", "#4ade80", "#fbbf24", "#f87171"]); 
    ...
    const tooltip = d3
      ...
      .attr(
        "class",
        "bg-gray-800 text-white p-2 rounded-lg text-sm absolute hidden"
      ) 
  ...
  return (
    ...
    >
           {" "}
      <Heading as="h2" mb={4} className="text-lg font-bold text-center">
        Intensity Chart
      </Heading>
           {" "}
      <Select
