...
const RelevanceBubbleChart = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // Add bubble groups for both datasets (Relevance & Intensity)
    ['relevance', 'intensity'].forEach((key, index) => {
      svg.selectAll(`.${key}-bubbles`)
        .data(data)
        .enter()
        .append('circle')
        ...
        .attr('r', d => rScale(key === 'relevance' ? d.relevance * 5 : d.intensity))
        .attr('fill', index === 0 ? '#6366F1' : '#22C55E')  // Different colors for Relevance and Intensity
        .attr('opacity', 0.8)
        .attr('stroke', 'white')
        .attr('stroke-width', 1)
        .on('mouseover', function (event, d) {
          d3.select(this)
            ...
            .attr('opacity', 1)
            .attr('stroke-width', 2)
            .attr('r', rScale(key === 'relevance' ? d.relevance * 6 : d.intensity * 1.2));

          // Append white hover tooltip with smooth transition
          svg.append('text')
            .attr('id', 'tooltip')
        ...
        .on('mouseout', function () {
          d3.select(this)
            ...
            .attr('opacity', 0.8)
            .attr('stroke-width', 1)
            .attr('r', rScale(key === 'relevance' ? data.relevance * 5 : data.intensity));
...
          Relevance Chart
        </Heading>      <svg ref={svgRef} className="w-full h-[600px]" />
    </div>
  );
};
